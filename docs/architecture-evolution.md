...
This document describes implementation-strategy evolution, not the product-capability backlog. Manual platform moderation, group moderation, and provider-neutral moderation signals are already implemented; advanced entries below describe optional implementation strategies around the current authority model.

## Configuration rule

Keep two kinds of configuration separate:

- **capability flags** answer _what is enabled?_ and remain in `SOCIAL_FEATURES` (`profiles`, `media`, `posts`, `comments`, `reactions`, `votes`, `follows`, `follow_requests`, `saves`, `blocks`, `mutes`, `groups`, `chat`, and opt-in `moderation`);
...
| Automated moderation | manual Rust-owned moderation plus immutable provider-neutral signals; no automatic enforcement | explicit policy adapters may translate trusted signals into the ordinary audited moderation commands | `SOCIAL_MODERATION_AUTOMATION=off|policy` |
| Social filtering | shared Rust/SQL policy predicates over authoritative PostgreSQL relationships and moderation state | optional policy cache or derived graph that reproduces the same semantics | `SOCIAL_FILTERING_MODE=direct|cached` |
...
See `docs/social-capabilities.md` for the explicit implemented-versus-planned status of social semantics. Tree-shaped comments, reactions, saves/bookmarks, votes, and blocks/mutes are implemented; reposts and mentions/domain events remain planned.
