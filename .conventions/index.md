...
- **DB-001 — Keep durable records migration-friendly** — Complex durable records should normally include creation/update timestamps and a version when those fields materially help model changes, synchronization, or migration. ([details](modules/databases/technologies/databases/README.md))
- **DB-002 — Evolve persistent schemas through explicit migrations** — Production, staging, and other persistent environments change schema through committed migrations rather than implicit startup mutation. ([details](modules/databases/technologies/databases/README.md))
- **DB-003 — Keep test and seed data minimal and deterministic** — Tests create only the records they need through reusable builders/factories and deterministic IDs, clocks, and ordering where relevant. ([details](modules/databases/technologies/databases/README.md))
...
- **AUTHN-001 — A session identifies an account, not an authorization context** — Authenticate a stable account, not a profile, space, membership, or role. ([details](modules/security/conventions/authentication/README.md))
- **AUTHN-002 — One-time authentication secrets are non-recoverable credentials** — Generate secrets securely; bind them to a subject and purpose; store only a digest where possible. ([details](modules/security/conventions/authentication/README.md))
- **AUTHZ-001 — Every role assignment has an explicit authority scope** — Keep system and space role assignments distinct and scope-qualified. ([details](modules/security/conventions/authorization/README.md))
- **AUTHZ-002 — Authorize every protected operation at the server boundary** — Deny by default using current actor, action, resource, relationships, and context. ([details](modules/security/conventions/authorization/README.md))
- **AUTHZ-003 — Personal profiles and spaces are authorization resources** — Model profiles, spaces, memberships, ownership, and containment as explicit authoritative relationships. ([details](modules/security/conventions/authorization/README.md))
...
- [modules/databases/technologies/databases/README.md](modules/databases/technologies/databases/README.md)

### postgres

- [modules/postgres/technologies/databases/postgres/README.md](modules/postgres/technologies/databases/postgres/README.md)
...
- [modules/security/conventions/authentication/README.md](modules/security/conventions/authentication/README.md)
- [modules/security/conventions/authorization/README.md](modules/security/conventions/authorization/README.md)
